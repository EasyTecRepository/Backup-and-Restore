...
#
# Version 1.0.8
# nur fuer backupskript Versionen ab 1.2.8 zugelassen!
# Version BETA
#
#
...
# Standard-Pfad cache(vor-pfad)
# Wenn cache eingerichtet ist, MUSS dieser Pfad ebenfalls ein Backup erhalten.
# Im Cache werden wichtige Daten der Docker gespeichert. Ohne diese könnte der Docker nicht laufen!
hauptPfadCache="/mnt/cache"
...
# hauptPfadCache
if [ -d "${backupServerZielPfad}/${hauptPfadCache}" ]
  then
    echo "<font color='green'>OK</font>"
  else
    echo "<font color='red'>ERROR: Vezeichnis von "${hauptPfadCache}" existiert nicht! Breche ab...</font>"
    echo "<font color='orange'>Nutzt du auf Unraid einen cache? Wenn nicht, ist dies das falsche Skript!</font>"
     exit 1
fi
...
# rsync hauptPfadCache
echo "<font color='blue'>rsync von "${hauptPfadCache}"...</font>"
sudo rsync -az --progress "${rsync_user}"@"${backupServerIP}":"${backupServerZielPfad}" "${hauptPfadCache}"
echo
