...
if [ -d "${hauptPfadCache}" ]
  then
    ...
    echo "<font color='yellow'>Nutzt du auf Unraid einen cache? Wenn nicht, ist dies das falsche Skript!</font>"
     exit 1
fi
...
sudo rsync --ignore-existing -az "${hauptPfadCache}" "${rsync_user}"@"${backupServerIP}":"${backupServerZielPfad}"
echo
