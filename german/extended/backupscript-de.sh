...
# Es werden auch Backups einer MariaDB durchgeführt. Zudem werden Discord messages versendet.
...
echo "<font color='blue'>backup der "${DockerName_dumps_1}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_1}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_1}" > "${mariadb_pfad_dump_datenbank_1}"/${currentDate}_dump.sql
...
echo "<font color='blue'>backup der "${DockerName_dumps_2}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_2}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_2}" > "${mariadb_pfad_dump_datenbank_2}"/${currentDate}_dump.sql
...
echo "<font color='blue'>backup der "${DockerName_dumps_3}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_3}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_3}" > "${mariadb_pfad_dump_datenbank_3}"/${currentDate}_dump.sql
...
echo "<font color='blue'>backup der "${DockerName_dumps_4}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_4}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_4}" > "${mariadb_pfad_dump_datenbank_4}"/${currentDate}_dump.sql
...
echo "<font color='blue'>backup der "${DockerName_dumps_5}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_5}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_5}" > "${mariadb_pfad_dump_datenbank_5}"/${currentDate}_dump.sql
...
echo "<font color='blue'>backup der "${DockerName_dumps_6}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_6}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_6}" > "${mariadb_pfad_dump_datenbank_6}"/${currentDate}_dump.sql
...
echo "<font color='blue'>backup der "${DockerName_dumps_7}" Datenbank...</font>"
touch "${mariadb_pfad_dump_user_7}"/${currentDate}_dump.sql
docker exec "${DockerName_dumps_mariadb}" /usr/bin/mysqldump --user=root --password=${mariadbPassWD} "${mariadb_dumps_datenbank_7}" > "${mariadb_pfad_dump_datenbank_7}"/${currentDate}_dump.sql
...
## Test Dump 1 ##
if [ -f "${mariadb_pfad_dump_user_1}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName1}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi

echo 

## Test Dump 2 ##
if [ -f "${mariadb_pfad_dump_user_2}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName2}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi
...
## Test Dump 3 ##
if [ -f "${mariadb_pfad_dump_user_3}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName3}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi
...
## Test Dump 4 ##
if [ -f "${mariadb_pfad_dump_user_4}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName4}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi
...
## Test Dump 5 ##
if [ -f "${mariadb_pfad_dump_user_5}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName5}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi
...
## Test Dump 6 ##
if [ -f "${mariadb_pfad_dump_user_6}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName6}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi
...
## Test Dump 7 ##
if [ -f "${mariadb_pfad_dump_user_7}"/${currentDate}_dump.sql ]
  then
    echo "<font color='green'>Das "${DockerName7}" Backup wurde erfolgreich im angegebenen share abgelegt.</font>"
  else
    echo "<font color='red'>ERROR: Irgendetwas ist schiefgelaufen. Das Backup befindet sich nicht im angegebenen Pfad!</font>"
fi
...
## Abschließen mariadb dumps ##
echo "<font color='green'>Die MariaDB dumps wurden gespeichert.</font>"
